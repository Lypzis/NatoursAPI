    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'data' ? 'http://localhost:3000/api/v1/users/updateMe' : 'http://localhost:3000/api/v1/users/updatePassword';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibHlwemlzIiwiYSI6ImNrYnpqMHhseTBkMXgycW8xdHZzcTFrd3UifQ.IXJy9TB-oBaysLEZv6B_Pw';
  var map = new mapboxgl.Map({
    container: 'map',
    // this is an element id required
  ...
  locations.forEach(function (location) {
    // Create marker
    var el = document.createElement('div'); // check mozilla dev for 'createElement' :D
...
var stripe = Stripe('pk_test_51H4Rl3Bcp4rXuvfvWroOnhzKeENB9P5JdJuODAgrKCQoSFviUBLOOYeYnnS7fT2eZo07jeUQVLU4IEODIf8gJNqs00uWs8byPm');
...
var passwordCurrent = document.getElementById('password-current');
...
if (mapBox) {
  ...
  (0, _mapbox.displayMap)(locations); // REMEMBER: for bug reasons mapbox module can't yet be used along with parcel
}
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            event.preventDefault();
            submitButton = document.querySelector('.btn--save-password');
...
if (bookButton) bookButton.addEventListener('click', function (event) {
  event.preventDefault();
  event.target.textContent = 'Processing...';
  var tourId = event.target.dataset.tourId; // tour-id is automatically converted to tourId
